...
/**
 * A class to walk through directories both sync and async. Use the walk
 * method to start a walk. If you pass an encoding, additionally all files are
 * read. You can listen to the following descriptive events:
 *
 * - fileOrDir: When a file or directory has been found. The callback is called
 * with callback(path, stat, basePath, depth, encoding).
 *
 * - file: When a file has been found. The callback is called with
 * callback(path, stat, basePath, depth, encoding).
 *
 * - dir: When a directory has been found. The callback is called with
 * callback(path, stat, basePath, depth, encoding).
 *
 * - end: When all directories and files have been found in the specified
 ...
 * basePath-param to distinguish the walks.
 *
 * - fileRead: After a file has been read. The callback is called with
 * callback(path, data, basePath, depth, encoding).
 *
 * - dirRead: After a dir has been read. The callback is called with
 * callback(path, files, basePath, depth, encoding).
 *
 * - idle: A special event that is fired after the last walk has ended.
 * Additionally if you add a listener and there is no walk currently running,
 * the listener is called instantly.
 *
 * - error: If an errors occurs the callback is called with
 * callback(err, path, basePath, depth, encoding).
 *
...
function Finder() {
    ...
    /**
     * A function to filter unwanted files. If the function returns true,
    ...
    /**
     * A function to filter unwanted directories. If the function returns true,
     * the directory is accepted. The function is called with the directory name.
     *
    ...
    /**
     * Returns the number of walks that are currently running.
     *
    ...
    /**
     * Starts the walk at the specified path. You should call this function
     * AFTER you have added all events. To start an recursive walk you can
     * pass Finder.RECURSIVE as depth or just omit it.
     *
     * In case you pass an encoding, all files are read.
     *
    ...
    /**
     * Convenient function that starts the walk after all currently running
     * walks have ended.
     *
    ...
    /**
     * Stops a specific walk or all walks if no path is passed.
     *
     * NOTE: This function does not fire the "end"-event.
     *
    ...
    /**
     * The synchronous version of walk().
     *
    ...
    /**
     * Removes all filters and listeners and stops all walks immediately.
...
/**
 * A constant to pass as depth value. This value indicates a recursive walk.
 *
