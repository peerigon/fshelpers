...
exports.makeDir = testCase({
    makeDir: function(test) {
        ...
        function finished(error) {
            ...
            if(times === 2) {
                test.ok(fs.existsSync(__dirname + "/async/folder"));
                test.ok(fs.existsSync(__dirname + "/async/folder/folder1"));
                test.ok(fs.existsSync(__dirname + "/async/folder/folder2"));
                test.ok(fs.existsSync(__dirname + "/async/folder/folder2/folder3"));
                test.done();
            }
        }
...
exports.makeDirSync = testCase({
    makeDir: function(test) {
        ...
        test.ok(fs.existsSync(__dirname + "/sync/folder"));
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder1"));
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder2"));
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder2/folder3"));
        test.done();
    }
});
...
exports.makeDirs = testCase({
    makeDirs: function(test) {
        function finished(errors) {
            ...
            test.ok(fs.existsSync(__dirname + "/async/folder/folder1/folder1"));
            test.ok(fs.existsSync(__dirname + "/async/folder/folder1/folder2"));
            test.ok(fs.existsSync(__dirname + "/async/folder/folder1/folder3"));
            test.ok(fs.existsSync(__dirname + "/async/folder/folder2/folder1"));
            test.ok(fs.existsSync(__dirname + "/async/folder/folder2/folder2"));
            test.ok(fs.existsSync(__dirname + "/async/folder/folder2/folder3"));
            test.done();
        }
...
exports.makeDirsSync = testCase({
    makeDirsSync: function(test) {
        ...
        test.strictEqual(errors, null);
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder1/folder1"));
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder1/folder2"));
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder1/folder3"));
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder2/folder1"));
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder2/folder2"));
        test.ok(fs.existsSync(__dirname + "/sync/folder/folder2/folder3"));
        test.done();
    }
});
